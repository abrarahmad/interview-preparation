# Interview preparation
#

This repository contains a comprehensive collection of interview questions and answers covering .NET Full Stack development, including C#, ASP.NET Core, Angular, Azure, SQL Server, Microservices, System Design, and related technologies. It is designed to help developers prepare for technical interviews ranging from Senior Developer to Technical Lead and Architect roles.

## 📚 Read the Complete Wiki

The full interview guide is available in the GitHub Wiki:

➡️ **[Angular](../../wiki/Angular)**

➡️ **[Angular Common Problems and Solutions](../../wiki/Angular-Common-Problems-and-Solutions)**

➡️ **[C‐Sharp-(C#)](../../wiki/C‐Sharp-(C%23))**

➡️ **[.NET](../../wiki/.NET)**

➡️ **[Linq](../../wiki/Linq)**

➡️ **[Entity Framework Core](../../wiki/Entity-Framework-Core)**

➡️ **[.NET & Azure](../../wiki/.NET-&-Azure)**

➡️ **[Azure Problem-Solutions](../../wiki/Azure-Problem-Solutions)**

➡️ **[Azure Functions](../../wiki/Azure-Functions)**

➡️ **[CI/CD](../../wiki/CI-CD)**

➡️ **[CI/CD Problems and Solutions](../../wiki/CI-CD-Problems-and-Solutions)**

➡️ **[System Design Problems & Solutions](../../wiki/System-Design-Problems-&-Solutions)**

➡️ **[System Design ‐ Complete Learning Roadmap](../../wiki/System-Design-‐-Complete-Learning-Roadmap)**


## ⭐ Contributing

Feel free to suggest improvements, add new questions, or update answers for newer versions.

## 📖 Start Reading

Visit the Wiki to access all interview questions and detailed answers:

➡️ **[Open Wiki](../../wiki)**
